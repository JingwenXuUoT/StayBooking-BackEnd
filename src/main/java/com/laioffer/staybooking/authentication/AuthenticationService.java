...
public class AuthenticationService {
    ...
    //no need @Transactional, only one write operation
    public UserEntity register(String username, String password, UserRole role) {
        if (userRepository.existsByUsername(username)) {
            throw new UserAlreadyExistException();
        }

        UserEntity userEntity = new UserEntity(null, username, passwordEncoder.encode(password), role);//customized userEntity, make registration more easy
    ...
    }//session-based auth give login API, but token-based auth need to verify token before login
}
