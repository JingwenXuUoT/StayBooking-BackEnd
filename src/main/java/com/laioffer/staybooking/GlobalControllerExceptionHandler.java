...
@ControllerAdvice//assist controller,a huge try-catch, controller become more clean
public class GlobalControllerExceptionHandler {

    @ExceptionHandler(EntityNotFoundException.class)//throw by database
    public final ResponseEntity<ErrorResponse> handleException(EntityNotFoundException e) {
        return new ResponseEntity<>(new ErrorResponse(
                "Resource not found",
                "resource_not_found"),
                HttpStatus.NOT_FOUND);//potential credential risk solution: change status code
        //or hide real number in url from client
    }
