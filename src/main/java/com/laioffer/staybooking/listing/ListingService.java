...
public class ListingService {
    ...
    public List<ListingDto> getListings(Long hostId) {
        return listingRepository.findAllByHostId(hostId)
                .stream()
                .map(ListingDto::new)//match data structure
                .toList();
    }
    ...
    public void createListing(
        ...
        List<String> uploadedUrls = images.parallelStream()//change list to instance sequence, and split one thread to multiple threads
                .filter(image -> !image.isEmpty())//all the images filter at the same time
                .map(imageStorageService::upload)//map image file to url link
                .toList();
    ...
    public List<ListingDto> search(
        ...
        return listingRepository.searchListings(
                ...
                .stream()
                .map(ListingDto::new)
                .toList();//change listing entity into listing dto
